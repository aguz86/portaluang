...
import {
  LayoutDashboard, ChevronRight, Calculator, ReceiptText, CalendarDays, Target, CreditCard,
  TrendingUp, DollarSign, PieChart, BookOpen, Plus, Sparkles, HardDrive, Download,
  RotateCcw, Palette, Settings, User, Bell, AlertTriangle, Calendar, Info, X, Clock, Zap, Crown, Menu
} from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const notifications = React.useMemo(() => {
    const notifs: { id: number; title: string; desc: string; type: string; icon: React.ReactNode; time: string; path?: string }[] = [];
    let notifId = 1;
    
    // Check due bills
    const dueBills = bills.filter(b => !b.isPaid);
    if (dueBills.length > 0) {
      notifs.push({
        id: notifId++,
        title: 'Tagihan Belum Lunas',
        desc: `Ada ${dueBills.length} tagihan yang belum lunas.`,
        type: 'warning',
  ...
  return (
    <>
      <header className="bg-stone-900 border-b border-stone-800 sticky top-0 z-50 w-full shadow-sm">
        ...
        <div className="border-t border-stone-800/50 bg-stone-900/95 backdrop-blur-sm">
